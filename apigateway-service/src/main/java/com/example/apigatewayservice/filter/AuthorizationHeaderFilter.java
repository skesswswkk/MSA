...
public class AuthorizationHeaderFilter extends AbstractGatewayFilterFactory<AuthorizationHeaderFilter.Config> {
    ...
    //login -> token 반환 -> GET(with token) 요청 -> 서버에서 Header(include token) 확인
    @Override
    ...
    private boolean isJwtVaild(String jwt) {
        ...
        String sub = null;
        try{
            sub = Jwts.parser().setSigningKey(env.getProperty("token.secret"))
    ...
    //Spring WebFlux 데이터타입 2가지 : Mono(단일), Flux(다중)
